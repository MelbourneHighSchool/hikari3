...
class Defender:
    ...
    def _process_frame(self, rgb_array, detected_objects):
        ...
        try:
            ...
            # Get distance to own goal (yellow goal)
            ...
            if ball_special_found:
                ...
                if ball_special_x < 20 and -50 < ball_special_y < 50:
                    ball_captured = True
            ...
            own_goal_distance_mm = None
            own_goal_angle = 180
            
            if self.targetGoal == "blueGoal":
                own_goal_distance_mm = self.subsystems["ObjectDetection"].blueGoalClosestDistanceMM
                own_goal_angle = blue_goal_angle
            elif self.targetGoal == "yellowGoal":
                own_goal_distance_mm = self.subsystems["ObjectDetection"].yellowGoalClosestDistanceMM
                own_goal_angle = yellow_goal_angle

            print(f"Own goal angle: {own_goal_angle:.2f} distance: {own_goal_distance_mm:.2f}")
            ...
            if ball_captured:
                ...
                if ball_position is not None or special_processing == True:
                    ...
                    linering = self.components["LineRing"]

                    line_angle = 0

                    if line_result['detected']:
                        line_angle = line_result['angle']
                        print(f"Line detected at {line_angle:.2f}°, magnitude: {linering.magnitude:.2f}")


                    line_angle += compass_angle

                    right_angle = min((line_angle + 90 + 360) % 360, (line_angle - 90 + 360) % 360)
                    left_angle = max((line_angle + 90 + 360) % 360, (line_angle - 90 + 360) % 360)
                    ...
                    if -0.5 < ball_angle < 0.5:
                        # speed = 0.0
                        pass


                    elif ball_angle > 40:
                        horizontal_component += 5

                    elif ball_angle < -40:
                        horizontal_component -= 5

                    elif ball_angle > 30:
                        horizontal_component += 4
                    ...
                    # target distance is 150mm

                    if own_goal_distance_mm is not None:
                        offset = own_goal_distance_mm - 220

                        if offset > 0:
                            vertical_component -= 0.1
                        elif offset < 0:
                            vertical_component += 0.1

                        if offset > 30:
                            vertical_component -= 0.2
                        elif offset < -30:
                            vertical_component += 0.2

                        if offset > 40:
                            vertical_component -= 0.6
                        elif offset < -40:
                            vertical_component += 0.6

                    if own_goal_angle > 180:
                        own_goal_angle -= 360
                    elif own_goal_angle < -180:
                        own_goal_angle += 360
                    ...
                    if horizontal_component > 0:
                        # going right
                        if 0 > own_goal_angle > -150:
                            horizontal_component = -0 # dont go too far

                        if not line_result['detected']:
                            if 0 > own_goal_angle > -155:
                                horizontal_component = -0
                    ...
                    # vertical component is forward and backward
                    # horizontal component is left and right

                    # forward should be 0 degrees

                    output_angle = math.atan2(horizontal_component, vertical_component)
                    output_angle = output_angle * 180 / math.pi

                    if output_angle > 180:
                        output_angle -= 360
                    elif output_angle < -180:
                        output_angle += 360

                    direction = output_angle
                    speed = math.sqrt(horizontal_component ** 2 + vertical_component ** 2)
