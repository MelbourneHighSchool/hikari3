...
class Barebones:
    def __init__(self, components, subsystems):
        ...
        # Initialize PIDs
        self.compass_pid = PID(0.035, 0.0, 0.0, setpoint=0, output_limits=(-1.5, 1.5))
        self.target_goal_pid = PID(0.035, 0.0, 0.0, setpoint=0, output_limits=(-0.5, 0.5))
        ...
        self.line_touch_duration = 0.3  # Duration in seconds to maintain zero speed
...
##
            ...
            if tg_angle != None:
                tg_v = self.target_goal_pid(-tg_angle)
            ...
            if ball_position is not None:
                ...
                if -30 < ball_angle < 30:
                    direction = ball_angle + (-25 if ball_angle > 0 else 25)
                    ...
                    if real_distance < 250:
                        ...
                        if tg_angle != None:
                            print('try to turn to goal', tg_v) 
                            rotation = tg_v
                        else:
                            print('tg cant see')
                ...
                elif ball_angle > 20 or ball_angle < -20:
                    if real_distance > 200:
                        direction = ball_angle + (30 if ball_angle > 0 else -30)
                        speed = 3
                    else:
                        direction = (90 if ball_angle > 0 else -90)
                        speed = 1
                if ball_angle > 45 or ball_angle < -45:
                    if real_distance < 200:
                        direction = (90 if ball_angle > 0 else -90)
                        speed = 3
                    else:
                ...
                if ball_angle > 90 or ball_angle < -90:
                    direction = 180
                    speed = 3

                if ball_angle > 160 or ball_angle < -160:
                    direction = 135
                    speed = 3
            ...
            #     print("Line touched! Stopping for 2 seconds")
