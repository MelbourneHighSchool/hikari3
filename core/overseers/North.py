...
import cv2
from subsystems.ObjectDetection import pixels_to_mm, polar_to_components
...
class North:
    def __init__(self, components, subsystems):
        ...
        self.ball_memory_duration = 1  # Remember ball position for 0.5 second
    ...
    def _process_frame(self, rgb_array, detected_objects):
        ...
        try:
            ...
            object_detection_end = time.time()




            # goal stuff
            goal_processing_start = time.time()
            ...
            goal_processing_end = time.time()

            #
            
            # If we don't have the ball, try to get it
            ball_processing_start = time.time()
            ...
            if ball_captured:
                ...
                if target_goal_found:
                    ...
                    # If facing goal within 10 degrees, shoot
                    if abs(target_goal_angle) < 7:
                        kick = True
                        set_dribbler_speed = 0
                ...
                if ball_position is not None or special_processing == True:
                    ...
                    if special_processing == False:
                        ...
                        ball_angle = ball_angle - compass_angle # IN THIS CODE, BALL ANGLE IS NORTHED
                        ...
                        # write ball angle into bottom right but a bit above the bottom
                        cv2.putText(rgb_array, f"ba: {ball_angle:.1f}", (rgb_array.shape[1] - 100, rgb_array.shape[0] - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 1)

                        print(f"ball_angle: {ball_angle}")
                    

                    # IMPROVE SPECIAL_PROCESSING BALL_ANGLE LATER
                        ...
                        # write text indicating the special angle in the bottom right
                        cv2.putText(rgb_array, f"sa: {ball_angle:.1f}", (rgb_array.shape[1] - 100, rgb_array.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
                    ...
                    # self.subsystems["Drivetrain"].quickdrive(0, 0, -1 if ball_angle < 0 else 1)

                    ball_components = polar_to_components(ball_angle, ball_distance_mm)
                    ...
                    ball_comp_x, ball_comp_y = ball_components

                    cv2.putText(rgb_array, f'c: {ball_comp_x}, {ball_comp_y}', (50, 390), 
                                cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 1)


                    forwardback_movement = 0
                    leftright_movement = 0 

# the reasoning of components is by isolating the horizontal component we can more accurately setpoint it to 0
                    if -10 <= ball_comp_x <= 10:
                        leftright_movement = 0
                    
                    if ball_comp_x > 10:
                        leftright_movement = -1
                    ...
                    # maybe we can change the angle using the vertical and horizontal component calculated by the northed angle


                    direction = resultant_angle
                    speed = resultant_speed
    ...
    #########
            ...
            compass_time = (compass_end - compass_start) * 1000
            object_detection_time = (object_detection_end - object_detection_start) * 1000
            goal_processing_time = (goal_processing_end - goal_processing_start) * 1000
            ball_processing_time = (ball_processing_end - ball_processing_start) * 1000
            movement_time = (movement_end - movement_start) * 1000
            
            print(f"Total: {processing_time * 1000:.2f}ms | Compass: {compass_time:.2f}ms | Object: {object_detection_time:.2f}ms | Goal: {goal_processing_time:.2f}ms | Ball: {ball_processing_time:.2f}ms | Movement: {movement_time:.2f}ms")
