...
VisionReadout = namedtuple("VisionReadout", ("time", "ball_dir", "ball_dist", "ygoal_dir", "bgoal_dir", "bot_x", "bot_y"))
VISION_READOUT_FMT = "fBBBBBB" # Time (monotonic), ball_dir, ball_dist, ygoal_dir, bgoal_dir, bot_x, bot_y

class Camera:
    def __init__(self, PORT=3000, resolution=(640, 640), frame_rate=30):
        self.PORT = PORT
        self.resolution = resolution
        self.frame_rate = frame_rate
        self.forward_angle = 0  # Add forward angle property

        self.readout_buf = bytearray(struct.calcsize(VISION_READOUT_FMT))
        self.picam2 = picamera2.Picamera2()
        self.picam2.configure(self.picam2.create_video_configuration(main={"size": resolution, "format": 'RGB888'}))
        self.picam2.controls.FrameRate = frame_rate
        self.picam2.pre_callback = self._callback
        self.picam2.start()

    def _callback(self, request):
        with picamera2.MappedArray(request, "main") as m:
            now = time.monotonic()
            ball_dir, ball_dist = self._find_ball(m.array)
            ygoal_dir = self._find_ygoal(m.array)
            bgoal_dir = self._find_bgoal(m.array)
            bot_x, bot_y = self._find_bot(m.array)
            struct.pack_into(VISION_READOUT_FMT, self.readout_buf, 0,
                             now, ball_dir, ball_dist, ygoal_dir, bgoal_dir, bot_x, bot_y)
    
    def _find_ball(arr: cv2.Mat):
        return ("angle: int 0-255", "distance: int 0-255 (0 = cannot find ball)")
    
    def _find_ygoal(arr: cv2.Mat):
        return "angle: int 0-255 (0 = goal not visible)"
    
    def _find_bgoal(arr: cv2.Mat):
        return "angle: int 0-255 (0 = goal not visible)"
    
    def _find_bot(arr: cv2.Mat):
        return ("x: int 0-255", "y: int 0-255")
    
    def get_vision_readout(self):
        cv2.cvtColor(self.picam2.capture_array("main"), cv2.COLOR_YUV2BGR, self.picam2.readout_buf)
        return VisionReadout(*struct.unpack_from(VISION_READOUT_FMT, self.readout_buf, 0))